...
if(@ARGV < 3){
	print "perl $0 <outdir> <calculat dir> <Sample>\n";
	exit ;
}
my $OutDir=shift;
my $CalcuDir=shift;
...
my %SmpidBam;
opendir D,"$OutDir/$Smp";

$SmpidBam{$Smp}="";
while(my $f=readdir(D)){
	chomp $f;
	next unless $f=~/.bam$/;
	$SmpidBam{$Smp}.=" $OutDir/$Smp/$f";
}

open QSUB,">$OutDir/qsub_MergeAndAnalysis.sh";

foreach my $Smpid(keys %SmpidBam){
	mkpath("$CalcuDir/$Smpid/");
	open S,">$CalcuDir/$Smpid/MergeAndCalculate.sh";

	(open Rsc,">$CalcuDir/$Smpid/$Smpid.plot_chr_cov.R") || die $!;
		print Rsc &generate_R("$CalcuDir/$Smpid",$Smpid);
	close Rsc;
...
#\$ -pe mpi 4
#\$ -l virtual_free=100M
#\$ -o $CalcuDir/$Smpid/$Smpid.o
#\$ -e $CalcuDir/$Smpid/$Smpid.e

## Merge Bam files
/share/app/samtools-1.2/bin/samtools merge $CalcuDir/$Smpid/$Smpid.bam $SmpidBam{$Smpid} 
/share/app/samtools-1.2/bin/samtools view -h $CalcuDir/$Smpid/$Smpid.bam | awk \'\{if \(\!\/\^\@\/ \&\& and\(\$2,4\) \=\= 4\) \{\$5=0\; \$6=\"\*\"\;\} gsub\(\/ \/\,\"\\t\"\,\$0\)\; print\}\' \| /share/app/samtools-1.2/bin/samtools view -b -S -T  $Ref->{$species} - > $CalcuDir/$Smpid/$Smpid.bf.bam
/share/Data01/lynn/projects/bin/java/jdk1.8.0_144/bin/java -Xmx3g -Djava.io.tmpdir=/share/Data01/tianwei/tmp/java_tmp/ -XX:MaxPermSize=512m -XX:-UseGCOverheadLimit -jar /share/Data01/tianwei/Bin/picard/FixMateInformation.jar I=$CalcuDir/$Smpid/$Smpid.bf.bam O=$CalcuDir/$Smpid/$Smpid.bftmp.bam TMP_DIR=/share/Data01/tianwei/tmp/java_tmp/ SO=coordinate VALIDATION_STRINGENCY=SILENT

##sort
/share/app/samtools-1.2/bin/samtools calmd -b $CalcuDir/$Smpid/$Smpid.bftmp.bam $Ref->{$species} > $CalcuDir/$Smpid/$Smpid.sort.bam
...
I=$CalcuDir/$Smpid/$Smpid.sort.bam O=$CalcuDir/$Smpid/$Smpid.sort.rmdup.bam \\
METRICS_FILE=$CalcuDir/$Smpid/$Smpid.duplicate.xls TMP_DIR=/share/Data01/tianwei/tmp/java_tmp/ VALIDATION_STRINGENCY=SILENT
/share/app/samtools-1.2/bin/samtools index $CalcuDir/$Smpid/$Smpid.sort.rmdup.bam
rm $CalcuDir/$Smpid/$Smpid.bam $CalcuDir/$Smpid/$Smpid.bf.bam $CalcuDir/$Smpid/$Smpid.bftmp.bam $CalcuDir/$Smpid/$Smpid.sort.bam

## get WGS coverage
/share/Data01/lynn/projects/bin/java/jdk1.8.0_144/bin/java -jar /share/app/picard-tools-1.137/picard.jar CollectWgsMetrics \\
I=$CalcuDir/$Smpid/$Smpid.sort.rmdup.bam \\
O=$CalcuDir/$Smpid/$Smpid.mainCov.xls \\
R=$Ref->{$species}

## plot coverage
/share/Data01/lynn/projects/bin/java/jdk1.8.0_144/bin/java -Xmx2G -Djava.io.tmpdir=/share/Data01/tianwei/tmp/java_tmp/ -XX:MaxPermSize=512m -XX:-UseGCOverheadLimit -jar /share/Data01/tianwei/Bin/TRIM_QC_4_SEQ500/qcmg/qprofiler-1.0.jar --log $CalcuDir/$Smpid/$Smpid.bam.qprofile.log --loglevel INFO --input $CalcuDir/$Smpid/$Smpid.sort.rmdup.bam --ntConsumer 2 --ntProducer 6 --output $CalcuDir/$Smpid/$Smpid.bam.qprofile.xml
/share/Data01/tianwei/Bin/R-3.2.1/bin/R CMD BATCH $CalcuDir/$Smpid/$Smpid.plot_chr_cov.R

## mapping stat
perl /share/Data01/tianwei/Bin/RNA_module/AlignmentStat/AlignmentStat_forBwa/bin/BwaMapStat.pl -bam $CalcuDir/$Smpid/$Smpid.sort.rmdup.bam -key $CalcuDir/$Smpid/$Smpid -samtools /share/app/samtools-1.2/bin/samtools
...
I=$CalcuDir/$Smpid/$Smpid.sort.rmdup.bam \\
O=$CalcuDir/$Smpid/insert_size_metrics.xls \\
H=$CalcuDir/$Smpid/insert_size_histogram.pdf \\
M=0.5

##copy result
cp $CalcuDir/$Smpid/$Smpid.duplicate.xls $CalcuDir/$Smpid/$Smpid.mainCov.xls $CalcuDir/$Smpid/insert_size_metrics.xls $CalcuDir/$Smpid/$Smpid.bam.qprofile.tsv $CalcuDir/$Smpid/$Smpid.qprofile.xml $CalcuDir/$Smpid/*.pdf $OutDir/$Smpid 
ln -s $CalcuDir/$Smpid/$Smpid.sort.rmdup.bam $OutDir/$Smpid
";

close S;

print QSUB "qsub $CalcuDir/$Smpid/MergeAndCalculate.sh\n";
}
...
parsed_xml_file <- \"$prefix.bam.qprofile.xml\"
