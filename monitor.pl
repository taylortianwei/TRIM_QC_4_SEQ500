...
mkpath("$FilePath{NDN}/$year_month_day");
...
foreach my $ff(readdir D_R){
	...
	my %summary; my %fqfiles;my %projects; my %BadFQ; my %main; my %DeMultiplex;
	...
	foreach my $_FLB(sort keys %projects){
		...
		if($fqfiles{$_FLB}){
			...
			my $tmpout= $FilePath{output}."/".$proj."/".$subproj."/".$zebra."_".$flowcell;
			open $OutPut{fqfiles}{"$proj|$subproj|$flowcell|$lane"},">>$FilePath{output}/$proj/$subproj/$zebra\_$flowcell/fqfiles.txt" unless $OutPut{fqfiles}{"$proj|$subproj|$flowcell|$lane"};
			print {$OutPut{fqfiles}{"$proj|$subproj|$flowcell|$lane"}} join("\t","$fqfiles{$_FLB}/$_FLB",$species,$realID,$SubmitID,$proj,$subproj,$Fq1Length,$Fq2Length,$Fq3Length),"\n";
			## Log Files
			...
			if($BadFQ{$_FLB} =~/^WL:(.*)/){
				...
				my $tmpout= $FilePath{output}."/".$proj."/".$subproj."/".$zebra."_".$flowcell."_".$status;
				open $OutPut{fqfiles}{"$proj|$subproj|$flowcell|$lane"},">>$FilePath{output}/$proj/$subproj/$zebra\_$flowcell\_$status/fqfiles.txt" unless $OutPut{fqfiles}{"$proj|$subproj|$flowcell|$lane"};
                                my $mc_type=$zebra; $mc_type=~s/\d+$//;
				$_FLB=~s/\|.*//;
                                print {$OutPut{fqfiles}{"$proj|$subproj|$flowcell|$lane"}} join("\t","$FilePath{datadir}/$mc_type$data/$zebra/$flowcell/$lane/$_FLB",$species,$realID,$SubmitID,$proj,$subproj,$Fq1Length,$Fq2Length,$Fq3Length),"\n";
	...
	foreach my $p(keys %main){
		...
		foreach my $subp(keys %$TmpHash){
			my $IfDep=$TmpHash->{$subp};

			my $tmpout="$FilePath{output}/$p/$subp/$zebra\_$flowcell";
			my $shell=&MainShell($Bin,$FilePath{output},$FilePath{CalcuDir},$FilePath{CopyDir},$tmpout,$p,$subp,$zebra,$flowcell,$summary{$flowcell},$IfDep);		
			open $OutPut{MainSH},">$FilePath{output}/$p/$subp/$zebra\_$flowcell/Main.sh" unless $OutPut{MainSH};
			print {$OutPut{MainSH}} $shell;
			## Run everything
...
sub MainShell
{
	my ($Bin,$output,$CalcuDir,$CopyDir,$tmpout,$p,$subp,$zebra,$flowcell,$s_summary,$IfDep)=@_;
...
perl $Bin/get_demultiplexing.pl $tmpout/fqfiles.txt $FilePath{barcodefile}
";
		my $check;
                foreach my $key(sort keys %$s_summary){
                        $shell.="cp -r $s_summary->{$key} $tmpout/1_SUMMARY\n";
